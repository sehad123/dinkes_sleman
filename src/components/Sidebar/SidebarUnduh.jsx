...
const SidebarUnduh = ({ title, sidebarStyle }) => {
  ...
  return (
    <div
      className={`lg:translate-x-0 translate-x-16 bg-white mt-10 border rounded-xl mx-10  dark:bg-gray-900 dark:text-white duration-200 sidebar-wrapper ${
    ...
    >
      <div className={`bg-white dark:bg-gray-900 w-64 mx-6 flex flex-col h-full py-4 ${sidebarStyle}`}>
        <div className={` flex justify-between items-center px-4 mb-4 ${!isSidebarOpen ? "ml-0" : ""}`}>
          <div className="text-3xl font-bold mb-2 mx-auto">{title}</div>
        </div>
        <div className={`dark:bg-gray-900 transition-all duration-300 ${isSidebarOpen ? "" : "hidden"} ${isSidebarOpen ? "bg-white" : "bg-gray-800"} ${!isSidebarOpen ? "-ml-64" : ""}`}>
          <ul className="text-lg mr-5 ">
            {menuItems.map((menuItem, index) => (
              <li
                key={index}
                className={`relative px-4 py-2 my-5 ${location.pathname === menuItem.link ? "bg-blue-400 rounded-lg dark:bg-gray-700" : " hover:bg-blue-400 hover:font-semibold  hover:rounded-lg dark:hover:bg-gray-700"}`}
                  ...
                  <ul className="mt-2 bg-white dark:bg-gray-800 border border-blue-400 dark:border-gray-600 rounded-lg shadow-lg py-1" onMouseEnter={() => setIsHoveredSubMenu(true)} onMouseLeave={() => setIsHoveredSubMenu(false)}>
                    {menuItem.dropdown.map((submenuItem, submenuIndex) => (
                      <li key={submenuIndex} className="px-4 py-2  hover:bg-gray-300  hover:rounded-lg">
                        <Link to={submenuItem.link} className={`block ${location.pathname === submenuItem.link ? " dark:text-gray-200 text-gray-500" : "text-gray-500 dark:text-gray-400 "}`}>
                          {submenuItem.name}
                        </Link>
                      </li>
                    ))}
